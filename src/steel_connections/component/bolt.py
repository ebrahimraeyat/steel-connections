...
class Bolt:
    ...
    def __post_init__(self):
        ...
        self.constr = json.dumps(
            {
                "d_f": self.d_f,
                "bolt_cat": self.bolt_cat,
                "threads_included": self.threads_included,
            }
        )
        ...
        self.a_e_min = (
            1.5 * self.d_f
        )  # AS4100:1998 Table 9.6.2 machine flame cut, sawn or planed edge plates
        self.s_p_min = 2.5 * self.d_f  # AS4100:1998 CL 9.6.1

        # self.phiV_f = self.phi_shear * (
        #     self.V_fn if self.threads_included else self.V_fx
        # )
    ...
    @property
    def standard_hole_diameter(self) -> float:
        if self.d_f <= 2.2:
            return self.d_f + 0.2
        else:
            return self.d_f + 0.3
    ...
    @property
    def standard_hole_diameter_short_lobiaee(self) -> float:
        if self.d_f <= 2.4:
            return self.d_f + 0.6
        else:
            return self.d_f + 1.0
    ...
    @property
    def nominal_shear(self) -> float:
        '''
        kg/ cm^2
        '''
        return 0.55 * self.f_uf
    
    @property
    def nominal_shear_force(self):
        return self.nominal_shear * self.A_o
...
class BoltGroup2D:
    ...
    def __post_init__(self):
        ...
        self.name = (
            f"{self.n_p} x {self.n_g} ({self.s_p}p x {self.s_g}g) {self.bolt.name}"
        )
        self.constr = json.dumps(
            {
                "n_p": self.n_p,
                "n_g": self.n_g,
                "bolt": json.loads(self.bolt.constr),
                "s_p": self.s_p,
                "s_g": self.s_g,
            }
        )
    ...
    def a_ey(self, a_ev_e):
        ...
        return min(
            a_ev_e - 1, self.a_ey_bc
        )  # min[vertical edge distance -1 , vertical hole edge to bolt centre]

    def a_ex(self, a_eh_e):
        """Horizontal plate tear-out length"""
        return min(
            a_eh_e - 1, self.a_ex_bc
        )  # min[horizontal edge distance -1, horizontal hole edge to bolt centre]
    ...
    def Z_b(self, e):
        if self.n_p != 1:
            Z_1 = (
                ...
                * e
                / self.s_g
                / (1 + 1 / 3 * ((self.n_p + 1) / (self.n_p - 1)) * (1 / self.s_pg) ** 2)
            )
            return (
                1 / ((1 + Z_1) ** 2 + (Z_1 / self.s_pg) ** 2) ** 0.5
            )  # if e=0 return 1
        elif self.n_p == 1:
    ...
    def I_bp(self):
        return (
            ...
            * self.n_p
            / 12
            * (self.s_p**2 * (self.n_p**2 - 1) + self.s_g**2 * (self.n_g**2 - 1))
        )
    ...
    def Z_eh(self, e):  # double
        if self.n_p != 1:
            return self.I_bp / (
                e * (self.n_p - 1) * self.s_p * self.n_p
            )  # also equal to Z_e + 3*self.s_g**2/self.s_p/(6*e*(self.n_p-1))
        elif self.n_p == 1:
            return 0

    def Z_ev(self, e):  # double
        if self.n_p != 1:
            return 1 / (1 + self.n_p * e * self.s_g / self.I_bp)  # if e=0 return 1
        elif self.n_p == 1:
            return self.s_g / (self.s_g + 2 * e)  # if e=0 return 1
